...
fn bench_expand_command_args(c: &mut Criterion) {
    let cmd = vec![
        ...
        "--labels=${LABELS:-team=infra,role=collector,env=prod}".to_string(),
    ];
...
fn sample_elf_with_interp(interpreter: &str) -> Vec<u8> {
    ...
    bytes[program_header + 8..program_header + 16].copy_from_slice(&interp_offset.to_le_bytes());
